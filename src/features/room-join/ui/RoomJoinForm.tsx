...
import { Member } from "@/entities/room"; // ✅ Member 타입 임포트 추가

interface Props {
  roomCode: string;
  onSuccess: (memberId: string, member: Member) => void; // ✅ member 객체 추가
}

export function RoomJoinForm({ roomCode, onSuccess }: Props) {
  const name = useRoomJoinStore((s) => s.name);
  const password = useRoomJoinStore((s) => s.password);
  ...
  const setPassword = useRoomJoinStore((s) => s.setPassword);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (result) {
      reset();
      onSuccess(result.memberId, result.member); // ✅ result.member 추가 전달
    }
  };
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder="나중에 내 정보 수정 시 사용할 비밀번호" // ✅ 개인 비밀번호임을 명시
              disabled={isSubmitting}
              className="h-12 w-full rounded-2xl bg-background px-4 text-sm text-gray-900 outline-none ring-1 ring-input transition focus-visible:ring-2 focus-visible:ring-primary/40"
              maxLength={20}
            />
            {password && (
              <ClearButton onClick={() => setPassword("")} className="right-3" />
            )}
          </div>
          <Button
            type="submit"
