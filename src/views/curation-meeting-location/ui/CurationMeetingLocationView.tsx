  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        // 초기 selected hydrate — 이미 확정된 모임 장소가 있으면 그것을, 없으면 멤버 출발지 평균
        if (status.room.meetingLocation) {
          const loc = status.room.meetingLocation;
          useLocationInputStore.getState().setSelected({
            label: loc.label,
          ...
          const membersWithLocation = status.members.filter((m) => m.location);
          if (membersWithLocation.length > 0) {
            const avgLat =
              membersWithLocation.reduce((s, m) => s + m.location!.lat, 0) /
              membersWithLocation.length;
            const avgLng =
              membersWithLocation.reduce((s, m) => s + m.location!.lng, 0) /
              membersWithLocation.length;
            useLocationInputStore.getState().setSelected({
              label: "선택한 위치",
              roadAddress: "선택한 위치",
